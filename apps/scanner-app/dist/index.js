...
async function handleScan(ticketId) {
    try {
        const data = await postJson("/scan", { ticketId });
        ...
        console.error(red(`Scan failed: ${err.message}`));
    }
}
async function handleLookup(query) {
    try {
        const { results } = await getJson(`/lookup?q=${encodeURIComponent(query)}`);
        if (!results.length) {
        ...
        await handleScan(answer.ticketId);
    }
    catch (err) {
        console.error(red(`Lookup failed: ${err.message}`));
    }
}
async function main() {
